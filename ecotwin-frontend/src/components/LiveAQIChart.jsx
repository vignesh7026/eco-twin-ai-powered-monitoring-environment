...
function LiveAQIChart({ lat = 12.9716, lon = 77.5946, cityLabel = "Bengaluru", hours = 12 }) {
  ...
  return (
    ...
    <div
      className="h-full flex flex-col bg-slate-900/40 backdrop-blur-xl border border-teal-400/10 rounded-3xl p-8 relative overflow-hidden transition-all duration-300"
    ...
    >
      ...
      {/* ── Header ── */}
      <div className="flex items-start justify-between gap-4 mb-6 relative flex-wrap shrink-0">
        <div>
      ...
      {!loading && !error && latest && (
        <div className="grid grid-cols-3 gap-4 mb-6 relative shrink-0">
          {/* Current AQI */}
      ...
      {/* Sync timestamp */}
      {lastUpdated && (
        <p className="text-[10px] font-mono uppercase tracking-widest text-slate-500 mb-4 relative shrink-0">
          Synced · {lastUpdated.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
        </p>
      )}

      {/* ── Chart ── */}
      {/* flex-1: fills remaining vertical space so the card height matches
          its sibling regardless of content — chart stretches, doesn't shrink. */}
      {loading ? (
        <div className="flex-1 min-h-[280px] bg-slate-900/60 border border-teal-400/10 rounded-2xl animate-pulse" />
      ) : error ? (
        <div className="flex-1 min-h-[280px] flex flex-col items-center justify-center gap-4 bg-slate-900/60 border border-rose-400/10 rounded-2xl">
